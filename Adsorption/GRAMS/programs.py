...
def LoadAmpTorch(kind, iteration):
    ...
    if iteration > 0:
        config['optim']['epochs'] = 1000
        config['dataset']['save_fps'] = False
        config['cmd']['identifier'] = 'ADS'+str(iteration)
        if kind == 'old': config['cmd']['logger'] = False
        else: config['cmd']['logger'] = True
...
def write_train(iteration):
    ...
    for i in uniqeindexsort:
        ...
        #skip unstable structures (eV)
            ...
            if c.get_potential_energy() < -30 and c.get_potential_energy() > -50: 
                train.write(c)
