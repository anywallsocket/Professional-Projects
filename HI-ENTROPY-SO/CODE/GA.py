...
if iteration == 0: calc = EMT()
else: calc = LoadSchNetCalc('NNs/'+str(iteration-1))

#elements = {'Ni':28, 'Cu':29, 'Pd':46, 'Ag':47, 'Pt':78, 'Au':79}
#selections = [NchooseM(atoms, 3) for i in range(stoichiometries)]
selections = np.linspace(1,atoms-1, stoichiometries, dtype=int)

print('Generating', stoichiometries, 'stoichiometries for', atoms,'atom clusters')
ALL = [] #Collect all images per bimetal per stoichiometry

###------------------------------------------
### BIMETAL LOOP
###------------------------------------------

bimetals = ['AgAu','AgCu','AgNi','AgPd','AgPt',
			'AuCu','AuNi','AuPd','AuPt',
			'CuNi','CuPd','CuPt',
			'NiPd','NiPt',
			'PdPt']

binumbers = [[47,79],[47,29],[47,28],[47,46],[47,78],
			[79,29],[79,28],[79,46],[79,78],
			[29,28],[29,46],[29,78],
			[28,46],[28,78], 
			[46,78]]

for b in range(len(bimetals)):
	...
	for s in selections:
		...
		stoichiometry = s*[binumbers[b][0]] + (atoms-s)*[binumbers[b][1]]
		print(N2S(stoichiometry))

		db_file = 'DBs/work-'+bimetals[b]+'-'+str(s)+'.db'

		if iteration == 0: pop_size = 100
		else:
			prev_dir = connect('DBs/'+str(iteration-1)+'/temp-'+bimetals[b]+'-'+str(s)+'.db')
		...
		print('Relaxing initial population...', end='\t')
		...
		write('DBs/'+str(iteration)+'/temp-'+bimetals[b]+'-'+str(s)+'.db', images)
...
#Filter and lognorm select
images = trim(prune(ALL, 3), 2)
idx, sample = normlog([i.get_potential_energy() for i in images], 100)
